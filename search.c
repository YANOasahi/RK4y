...
double caltmag(double xpos, double ypos, double mag_x, double mag_y, double mag_rad, double mcur, double *tcur ){
  ...
  for(itri = 0 ; itri<10 ; itri++){
    Btrim[itri] = (a1[itri]*exp(-((x2+off1[itri]-b1[itri])*(x2+off1[itri]-b1[itri]))/(2 * c1[itri] * c1[itri])))
      + (a2[itri]*exp(-((x2+off2[itri]-b2[itri])*(x2+off2[itri]-b2[itri]))/(2 * c2[itri] * c2[itri])))
      + (a3[itri]*exp(-((x2+off3[itri]-b3[itri])*(x2+off3[itri]-b3[itri]))/(2 * c3[itri] * c3[itri])))
      + (a4[itri]*exp(-((x2+off4[itri]-b4[itri])*(x2+off4[itri]-b4[itri]))/(2 * c4[itri] * c4[itri]))) ;
    }
...
double calfmag(double xpos, double ypos, double mag_x, double mag_y, double mag_rad, double mcur ){
  ...
  if(fabs(xpos1) < 0.2 && fabs(ypos1) < 1000){
    Bz=B0*(fw11 + fw21*xpos1 + fw31*xpos2 + fw41*xpos3 + fw51*xpos4 + fw61*xpos5+ fw71*xpos6+ fw81*xpos7+ fw91*xpos8 )
      *(1./(1+exp(en1 + en2*dy1 + en3*dy2 + en4*dy3 + en5*dy4 + en6*dy5 ))) ;
    Bz = Bz * -1 ;
  }
  else {
    Bz = 0. ;
